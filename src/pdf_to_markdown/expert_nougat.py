"""
Copied from https://github.com/facebookresearch/nougat/blob/main/predict.py
Change the suffix from .mmd to .md
"""
import sys
...
def main():
    ...
    for pdf in args.pdf:
        if not pdf.exists():
            continue
        if args.out:
            out_path = args.out / pdf.with_suffix(".md").name
    ...
    for i, (sample, is_last_page) in enumerate(tqdm(dataloader)):
        ...
        for j, output in enumerate(model_output["predictions"]):
            ...
            if is_last_page[j]:
                ...
                out = re.sub(r"\n{3,}", "\n\n", out).strip()
                if args.out:
                    out_path = args.out / Path(is_last_page[j]).with_suffix(".md").name
